    ...
    render(logs) {
        ...
        if (!logs.length) {
            container.innerHTML = `
                <div class="flex flex-col items-center justify-center py-16 text-neutral-500">
                    <p class="text-xs lg:max-xl:text-xs xl:text-base font-bold uppercase tracking-wider">Tidak ada log</p>
                </div>`;
            return;
        }
        ...
        container.innerHTML = logs.map((log, idx) => {
            ...
            if (log.category === "unknown") {
                return `<div class="border-b border-[#1c1c1c] py-3 px-4 text-xs lg:max-xl:text-xs xl:text-base text-neutral-500 font-mono">${Utils.escapeHtml(log.raw)}</div>`;
            }
            ...
            return `
                <div class="border-b border-[#1c1c1c]/50 py-3 px-4 hover:bg-[#121212] transition-colors ${idx % 2 === 0 ? 'bg-[#0a0a0a]' : ''}">
                    <div class="flex items-center justify-between text-[10px] lg:max-xl:text-xs xl:text-base mb-1">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span class="px-2 py-0.5 rounded text-[10px] lg:max-xl:text-xs xl:text-base font-mono text-neutral-300 bg-[#171717] border border-[#262626]" title="Operator">${Utils.escapeHtml(log.user)}</span>
                            ${log.ip_address && log.ip_address !== '-' ? `<span class="px-2 py-0.5 rounded text-[10px] lg:max-xl:text-xs xl:text-base font-mono text-sky-400 bg-[#0c2a3b] border-transparent" title="IP Address">${Utils.escapeHtml(log.ip_address)}</span>` : ''}
                        </div>
                        <span class="px-2 py-0.5 rounded text-[10px] lg:max-xl:text-xs xl:text-base font-medium border ${catColor}">${log.category}</span>
                    </div>
                    <div class="text-xs lg:max-xl:text-xs xl:text-base mb-1">
        ...
        const modalHtml = `
            <div class="text-left space-y-3">
                <div class="text-center">
                    <p class="text-xs lg:max-xl:text-xs xl:text-base text-neutral-200 font-bold uppercase tracking-wider">Bersihkan Semua Audit Log?</p>
                    <p class="text-[11px] lg:max-xl:text-xs xl:text-xs text-neutral-400 mt-1">Tindakan ini akan mengosongkan riwayat log aktif di dashboard.</p>
                </div>
