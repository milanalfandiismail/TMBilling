            ...
            const options = paketList.map((p, idx) => `
                <div class="flex items-center justify-between p-3 bg-[#141414] border border-[#2a2a2a] rounded-xl transition-all gap-4 select-item relative hover:border-neutral-500" data-paket-id="${p.id}">
                    ...
                    <div class="flex items-center bg-[#0a0a0a] border border-[#2a2a2a] rounded-lg overflow-hidden h-8 opacity-45 pointer-events-none transition-all shrink-0" id="qty-container-${p.id}">
                        <button onclick="Member.adjustPaketQty(${p.id}, -1)" class="w-7 h-full bg-[#1a1a1a] hover:bg-[#222] text-neutral-300 font-bold text-xs transition-colors flex items-center justify-center select-none">-</button>
                ...
                <div class="bg-[#111] border border-[#2a2a2a] rounded-xl p-4 md:p-6 max-w-4xl w-[calc(100%-2rem)] mx-auto md:w-full max-h-[85vh] overflow-y-auto scrollbar-thin my-auto shadow-2xl">
                    <div class="flex items-center justify-between mb-3 pb-2.5 border-b border-[#2a2a2a]">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-xs font-bold text-neutral-100 uppercase tracking-wider">Isi Waktu Billing</h3>
                                <p class="text-[9px] text-neutral-500 mt-0.5">Tambah saldo member (Multiple Paket)</p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-5">
                        ...
                        <div class="md:col-span-1 space-y-4">
                            <div class="bg-[#161616] border border-[#2a2a2a] rounded-lg p-4">
                                <div class="text-[9px] text-neutral-500 uppercase font-bold">Member</div>
                                <div class="font-bold text-xs text-neutral-200 mt-0.5 lg:truncate break-words whitespace-normal">${member.username}</div>
                                
                                <div class="border-t border-[#2a2a2a] my-3"></div>
                                
                                <div class="text-[9px] text-neutral-500 uppercase font-bold">Sisa Waktu Saat Ini</div>
                                <div class="font-bold text-xs text-neutral-100 font-mono mt-0.5">${Utils.formatDurasiFriendly(member.waktu_saved || member.waktu_tersimpan)}</div>
                            </div>
                    ...
                    <div class="flex gap-3 justify-end mt-5 pt-4 border-t border-[#2a2a2a]">
                        ...
                        <button onclick="Member.doAddWaktu(${memberId})" class="px-5 py-2.5 bg-neutral-100 hover:bg-[#e5e5e5] text-black text-xs font-bold rounded-lg transition-colors font-bold">Tambah Waktu</button>
                    </div>
                </div>`;
            Modal.show(html);
