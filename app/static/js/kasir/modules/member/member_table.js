        ...
        let tableHtml = `
                                ...
                                <tr class="hover:bg-[#121212] transition-colors block lg:table-row py-3 lg:py-0 border-b border-[#2a2a2a] last:border-b-0 lg:border-b-0">
                                    <td class="px-4 py-2 text-left block lg:table-cell">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded bg-[#171717] border border-[#262626] flex items-center justify-center text-neutral-200 font-bold text-xs lg:text-base">${m.username.charAt(0).toUpperCase()}</div>
                                            <div>
                                    ...
                                    <td class="px-4 py-2 text-center flex lg:table-cell justify-between items-center">
                                        ...
                                        ${m.aktif
                                            ? '<span class="inline-flex items-center gap-1 text-[9px] lg:text-base text-neutral-200 bg-[#171717] border border-[#262626] px-2 py-0.5 rounded font-bold"><span class="w-1 h-1 rounded-full bg-neutral-300 animate-pulse"></span>Aktif</span>'
                                            : '<span class="inline-flex items-center gap-1 text-[9px] lg:text-base text-neutral-500 bg-[#0c0c0c] border border-[#1c1c1c] px-2 py-0.5 rounded font-bold"><span class="w-1 h-1 rounded-full bg-neutral-600"></span>Terkunci</span>'}
                                    </td>
            ...
            <div class="flex items-center justify-between mt-4 px-2">
                ...
                <div class="flex items-center gap-2">
                    <button onclick="Member.gotoPage(${this.currentPage - 1})" class="px-3 py-1.5 bg-[#171717] border border-[#262626] hover:bg-[#222] text-neutral-400 text-xs lg:text-base font-bold rounded transition-colors ${!meta.has_prev ? 'opacity-30 cursor-not-allowed' : ''}" ${!meta.has_prev ? 'disabled' : ''}>&larr;</button>
                    <span class="px-4 py-1.5 bg-[#0c0c0c] border border-[#1c1c1c] rounded text-xs lg:text-base text-neutral-200 font-mono font-bold">${this.currentPage} / ${this.totalPages}</span>
                    <button onclick="Member.gotoPage(${this.currentPage + 1})" class="px-3 py-1.5 bg-[#171717] border border-[#262626] hover:bg-[#222] text-neutral-400 text-xs lg:text-base font-bold rounded transition-colors ${!meta.has_next ? 'opacity-30 cursor-not-allowed' : ''}" ${!meta.has_next ? 'disabled' : ''}>&rarr;</button>
                </div>
            </div>
        `;
