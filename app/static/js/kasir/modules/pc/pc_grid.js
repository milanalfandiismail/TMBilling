        ...
        if (groups.length === 0) {
            container.innerHTML = `
                <div class="flex flex-col items-center justify-center py-20 text-neutral-500 bg-[#0c0c0c] border border-dashed border-[#1c1c1c] rounded">
                    ...
                    <p class="text-xs lg:max-xl:text-xs xl:text-base font-bold uppercase tracking-wider">Belum Ada Unit PC</p>
                </div>`;
            return;
        }
                            ...
                            return `
                                <div class="bg-[#0c0c0c] border ${pc.aktif ? 'border-[#262626]' : 'border-[#1c1c1c] opacity-50'} rounded p-3 transition-colors hover:bg-[#121212] relative group">
                                    <div class="flex items-center justify-between mb-1.5">
                                        <span class="font-bold text-xs lg:max-xl:text-xs xl:text-base text-neutral-200">${pc.kode}</span>
                                    ...
                                    <div class="absolute inset-0 bg-[#0c0c0c]/95 rounded flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <button onclick="PC.delete(${pc.id})" class="w-7 h-7 lg:max-xl:w-7 lg:max-xl:h-7 xl:w-8 xl:h-8 rounded bg-[#171717] border border-[#262626] text-red-400 hover:bg-red-600 hover:text-white transition-colors flex items-center justify-center" title="Hapus PC">
