const Settings = {
    ...
    switchSubTab(subTab) {
        ...
        // Dokumentasi & Tutorial — load tutorials
        if (this.currentSubTab === 'tutorials') {
            this.loadTutorials();
        }
    },
    ...
    // ==========================================
    // DOKUMENTASI & TUTORIAL MANAGEMENT (CKEDITOR 5)
    // ==========================================
    tutorialsData: [],
    ...
    filterTutorials(category, btnEl) {
        ...
        document.querySelectorAll('.tutorial-cat-btn').forEach(btn => {
            btn.className = 'tutorial-cat-btn px-4 py-2 bg-[#0c0c0c] text-neutral-400 hover:text-white font-semibold text-xs sm:text-sm rounded-lg transition-all border border-[#1c1c1c]';
        });
        if (btnEl) {
            btnEl.className = 'tutorial-cat-btn px-4 py-2 bg-neutral-800 text-white font-bold text-xs sm:text-sm rounded-lg transition-all border border-[#262626]';
        }
        this.renderTutorialsList();
    },
    ...
    renderTutorialsList() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = `
                <div class="bg-[#0c0c0c] border border-[#1c1c1c] rounded-xl p-12 text-center text-neutral-500">
                    <span class="text-4xl block mb-2">📭</span>
                    <p class="font-semibold text-sm">Belum ada panduan untuk kategori ini.</p>
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = filtered.map(t => `
            <div class="bg-[#0c0c0c] border border-[#1c1c1c] rounded-xl p-6 space-y-4 shadow-lg hover:border-[#262626] transition-all">
                <div class="flex items-center justify-between border-b border-[#1c1c1c] pb-4">
                    <div class="flex items-center gap-3">
                        <span class="text-2xl">${t.icon || '🌐'}</span>
                        <div>
                            <h4 class="text-sm sm:text-base font-bold text-amber-400 uppercase tracking-wider">${t.title}</h4>
                            <span class="inline-block mt-1 text-[10px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded bg-neutral-900 border border-[#262626] text-neutral-400">
                                ${t.category}
                            </span>
                        </div>
                    </div>
                    ${isAdmin ? `
                    <div class="flex items-center gap-2">
                        <button onclick="Settings.openTutorialModal(${t.id})" class="px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-amber-400 text-xs font-semibold rounded-lg transition-colors border border-[#262626]">
                            ✏️ Edit
                        </button>
                        <button onclick="Settings.deleteTutorial(${t.id})" class="px-3 py-1.5 bg-rose-950/40 hover:bg-rose-900/60 text-rose-400 text-xs font-semibold rounded-lg transition-colors border border-rose-900/50">
                            🗑️ Hapus
                        </button>
                    </div>
                    ` : ''}
                </div>
                <div class="prose prose-invert max-w-none text-xs sm:text-sm text-neutral-300 leading-relaxed overflow-x-auto">
                    ${t.content}
                </div>
            </div>
        `).join('');
