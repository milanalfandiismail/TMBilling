const Blackout = {
    ...
    renderEmpty() {
        ...
        area.innerHTML = `
            <div class="flex flex-col items-center justify-center py-16 text-neutral-500 bg-[#0c0c0c] border border-dashed border-[#1c1c1c] rounded">
                ...
                <p class="text-xs lg:max-xl:text-sm xl:text-base font-bold uppercase tracking-wider text-neutral-300">Sistem Stabil</p>
                <p class="text-[10px] lg:max-xl:text-xs xl:text-base mt-1 text-neutral-500">Tidak ada sesi blackout yang terdeteksi.</p>
            </div>`;
    },
    ...
    renderList(list) {
        ...
        if (!area) return;
        if (list.length === 0) {
            area.innerHTML = '<div class="text-center text-neutral-500 py-12 text-xs lg:max-xl:text-xs xl:text-base">— Tidak Ada Riwayat Blackout —</div>';
            return;
        }
        ...
        if (pending.length > 0) {
            html += `
                <div class="mb-4 flex items-center gap-2">
                    ...
                    <h4 class="text-xs lg:max-xl:text-xs xl:text-base font-bold text-red-400 uppercase tracking-wider">Tertunda (${pending.length})</h4>
                </div>
        ...
        if (resolved.length > 0) {
            html += `
                <div class="mb-4 flex items-center justify-between">
                    <h4 class="text-xs lg:max-xl:text-xs xl:text-base font-bold text-neutral-400 uppercase tracking-wider">Selesai (${resolved.length})</h4>
                    <button onclick="Blackout.clearResolved()" class="text-xs lg:max-xl:text-xs xl:text-base text-red-400 hover:text-red-300 font-bold uppercase tracking-wider transition-colors">Bersihkan Log</button>
                </div>
                ...
                aksiHtml = `
                    <div class="flex gap-2">
                        <button onclick="Blackout.resolveMember(${s.id}, '${s.username}', ${s.sisa_waktu_mati})" 
                            class="px-3 py-1.5 bg-[#171717] hover:bg-neutral-100 border border-[#262626] text-neutral-300 hover:text-black text-xs lg:max-xl:text-xs xl:text-base font-bold rounded transition-colors">KEMBALIKAN ${s.sisa_waktu_mati}m</button>
                        <button onclick="Blackout.resolveGuestTutup(${s.id}, '${s.username}')" 
                            class="px-3 py-1.5 bg-[#2d1215] hover:bg-[#ef4444] border border-[#ef4444]/30 text-red-400 hover:text-white text-xs lg:max-xl:text-xs xl:text-base font-bold rounded transition-colors">TUTUP</button>
                    </div>`;
            } else {
                aksiHtml = `
                    <div class="flex gap-2 flex-wrap">
                        <button onclick="Blackout.resolveGuestSama(${s.id}, '${s.username}')" 
                            class="px-3 py-1.5 bg-[#0c0c0c] border border-[#1c1c1c] hover:bg-[#171717] text-neutral-300 text-xs lg:max-xl:text-xs xl:text-base font-bold rounded transition-colors">PC SAMA</button>
                        <button onclick="Blackout.showLanjutModal(${s.id}, '${s.username}', '${s.grup}')" 
                            class="px-3 py-1.5 bg-neutral-100 hover:bg-neutral-200 text-black text-xs lg:max-xl:text-xs xl:text-base font-bold rounded transition-colors">PINDAH PC</button>
                        <button onclick="Blackout.resolveGuestTutup(${s.id}, '${s.username}')" 
                            class="px-3 py-1.5 bg-[#2d1215] hover:bg-[#ef4444] border border-[#ef4444]/30 text-red-400 hover:text-white text-xs lg:max-xl:text-xs xl:text-base font-bold rounded transition-colors">TUTUP</button>
                    </div>`;
            }
        } else {
            aksiHtml = `<span class="text-xs lg:max-xl:text-xs xl:text-base font-bold text-neutral-400 flex items-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg> Teratasi</span>`;
        }
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="text-[9px] lg:max-xl:text-xs xl:text-base font-bold px-2 py-0.5 rounded bg-[#050505] text-neutral-400 border border-[#1c1c1c]">${isMember ? 'MEMBER' : 'TAMU'}</span>
                            <span class="text-xs lg:max-xl:text-xs xl:text-base font-bold text-neutral-200 lg:truncate break-words whitespace-normal font-mono">${s.username}</span>
                        </div>
            ...
            kosong.forEach(pc => {
                cardsHtml += `
                    <div class="pc-card-tujuan bg-[#050505] border border-[#1c1c1c] rounded p-3 cursor-pointer hover:border-neutral-500 transition-colors text-center" data-pc-id="${pc.id}">
                        <div class="font-bold text-neutral-200 font-mono text-xs lg:max-xl:text-xs xl:text-base">${pc.kode}</div>
                        <div class="text-[9px] lg:max-xl:text-xs xl:text-base text-neutral-500 font-mono">${pc.ip_address || '-'}</div>
                    </div>`;
            });
            cardsHtml += '</div>';
                ...
                <div class="bg-[#0c0c0c] border border-[#1c1c1c] rounded p-6 max-w-2xl w-[calc(100%-2rem)] mx-auto md:w-full">
                    <div class="flex items-center justify-between mb-4 pb-4 border-b border-[#1c1c1c]">
                        <div>
                            ...
                            <p class="text-[9px] lg:max-xl:text-xs xl:text-base text-neutral-500 mt-1">Guest: ${namaGuest} &middot; ${grup.toUpperCase()}</p>
                        </div>
                        <button onclick="Modal.closeModal()" class="text-neutral-500 hover:text-neutral-300 text-xl leading-none">&times;</button>
                    </div>
                    ${cardsHtml}
                    <div class="flex justify-end mt-4">
                        <button onclick="Modal.closeModal()" class="px-4 py-2 bg-[#171717] border border-[#262626] hover:bg-[#222] text-neutral-400 text-xs lg:max-xl:text-xs xl:text-base font-bold rounded transition-colors">Batal</button>
                    </div>
                </div>
            `);
